...
class FeaturesPage
{
    ...
    public function init_tabs()
    {
        $tabs = [new class('widgets') extends \DynamicContentForElementor\AdminPages\Features\GroupedListTab
        {
            ...
            {
                $features = \DynamicContentForElementor\Plugin::instance()->features->filter(['type' => 'widget']);
        ...
        }, new class('extensions') extends \DynamicContentForElementor\AdminPages\Features\GroupedListTab
        {
            ...
            public function get_all_tab_features()
            {
                $extensions = \DynamicContentForElementor\Plugin::instance()->features->filter(['type' => 'extension']);
                // We want all extension that are not dynamic tag or legacy
        ...
        }, new class('page-settings') extends \DynamicContentForElementor\AdminPages\Features\ListTab
        {
            ...
            public function get_all_tab_features()
            {
                $features = \DynamicContentForElementor\Plugin::instance()->features->filter(['type' => 'page-setting']);
                $features = \array_filter($features, function ($f) {
                    return !($f['legacy'] ?? \false);
                });
                return $features;
        ...
        }, new class('global-settings') extends \DynamicContentForElementor\AdminPages\Features\ListTab
        {
            ...
            public function get_all_tab_features()
            {
                $features = \DynamicContentForElementor\Plugin::instance()->features->filter(['type' => 'global-setting']);
                $features = \array_filter($features, function ($f) {
                    return !($f['legacy'] ?? \false);
                });
                return $features;
        ...
        }, new \DynamicContentForElementor\AdminPages\Features\FrontendNavigator(), new class('legacy') extends \DynamicContentForElementor\AdminPages\Features\GroupedListTab
        {
            ...
            public function get_groups()
            {
                return ['widget' => __('Widgets', 'dynamic-content-for-elementor'), 'extension' => __('Extensions', 'dynamic-content-for-elementor')];
            }
            public function get_all_tab_features()
            {
                return \DynamicContentForElementor\Plugin::instance()->features->filter(['legacy' => \true]);
            }
        }];
        $tabs_dict = [];
        foreach ($tabs as $tab) {
