...
update_option('WP-DCE-1_license_key', '4308eedb-1add-43a9-bbba-6f5d5aa6b8ee');
update_option('WP-DCE-1_license_expiration', '2030.06.01');
...
function dce_fail_load() {
	\DynamicContentForElementor\Notice::admin_notice__error( sprintf( __( 'You need %1$s"Elementor"%2$s for the %1$s"Dynamic.ooo - Dynamic Content for Elementor"%2$s plugin to work.', 'dynamic-content-for-elementor' ), '<strong>', '</strong>' ) );
}

function dce_admin_notice_minimum_elementor_version() {
	\DynamicContentForElementor\Notice::admin_notice__error( sprintf( __( 'Dynamic.ooo - Dynamic Content for Elementor requires Elementor version %1$s or greater.', 'dynamic-content-for-elementor' ), DCE_MINIMUM_ELEMENTOR_VERSION ) );
}

function dce_admin_notice_minimum_elementor_pro_version() {
	\DynamicContentForElementor\Notice::admin_notice__error( sprintf( __( 'If you want to use Elementor Pro with Dynamic.ooo - Dynamic Content for Elementor, it requires version %1$s or greater.', 'dynamic-content-for-elementor' ), DCE_MINIMUM_ELEMENTOR_PRO_VERSION ) );
}

function dce_admin_notice_old_php_version() {
	\DynamicContentForElementor\Notice::admin_notice__error( sprintf( __( 'You are using PHP version %1$s. This version is not more supported. Ask your provider to use PHP version %2$s+.', 'dynamic-content-for-elementor' ), phpversion(), DCE_PHP_VERSION_SUGGESTED ) );
}

function dce_admin_notice_suggest_new_php_version() {
	if ( isset( $_GET['page'] ) && 'dce-features' === $_GET['page'] ) {
		\DynamicContentForElementor\Notice::admin_notice__warning( sprintf( __( 'You are using PHP version %1$s. It\'s suggested to use PHP version %2$s+.', 'dynamic-content-for-elementor' ), phpversion(), DCE_PHP_VERSION_SUGGESTED ) );
	}
}
